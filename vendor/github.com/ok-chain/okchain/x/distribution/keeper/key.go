...
// gets the key for the validator snapshoot with address, okdex
func GetValidatorSnapshootKey(operatorAddr sdk.ValAddress) []byte {
	return append(ValidatorSnapshootPrefix, operatorAddr.Bytes()...)
}

// gets the key for the delegation snapshoot with address, prefix+valAddr+delAddr!, okdex
func GetDelegationSnapshootKey(delAddr sdk.AccAddress, valAddr sdk.ValAddress) []byte {
	return append(append(DelegationSnapshootPrefix, valAddr.Bytes()...), delAddr.Bytes()...)
}
