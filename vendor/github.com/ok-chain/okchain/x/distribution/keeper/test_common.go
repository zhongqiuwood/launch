...
func CreateTestInputAdvanced(t *testing.T, isCheckTx bool, initPower int64,
	...
	initCoins := token.ToUnit(initPower)
	...
	keyValSnap := sdk.NewKVStoreKey(types.ValidatorsKey) //okdex
	keyDelSnap := sdk.NewKVStoreKey(types.DelegationKey)

	db := dbm.NewMemDB()
	ms := store.NewCommitMultiStore(db)

	ms.MountStoreWithDB(keyDistr, sdk.StoreTypeIAVL, db)
	ms.MountStoreWithDB(keyValSnap, sdk.StoreTypeIAVL, db) //okdex
	ms.MountStoreWithDB(keyDelSnap, sdk.StoreTypeIAVL, db)
	...
	ctx := sdk.NewContext(ms, abci.Header{ChainID: "foochainid", Height: 1}, isCheckTx, log.NewNopLogger())
	...
	keeper := NewKeeper(cdc, keyDistr, keyValSnap, keyDelSnap, pk.Subspace(DefaultParamspace), ck, sk, fck, types.DefaultCodespace)
