...
func queryDelegationRewards(ctx sdk.Context, _ []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	//val, _ := k.GetValidator(ctx, params.ValidatorAddress)
	//del, _ := k.GetDelegation(ctx, params.DelegatorAddress, params.ValidatorAddress)
	//endingPeriod := k.incrementValidatorPeriod(ctx, val)
	rewards := k.calDelegationRewards(ctx, params.ValidatorAddress, params.DelegatorAddress)

	fmt.Println("params:", params, "rewards:", rewards)
...
func queryDelegatorTotalRewards(ctx sdk.Context, _ []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	k.IterateDelegations(
		ctx, params.DelegatorAddress,
		func(_ int64, del sdk.Delegation) (stop bool) {
			rewards := k.calDelegationRewards(ctx, del.GetValidatorAddr(), del.GetDelegatorAddr())
