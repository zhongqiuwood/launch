...
func NewMsgSubmitProposal(title, description string, proposalType ProposalKind, proposer sdk.AccAddress, initialDeposit sdk.DecCoins, params Params, height int64) MsgSubmitProposal {
...
func NewMsgDexListSubmitProposal(title, description string, proposalType ProposalKind, proposer sdk.AccAddress, initialDeposit sdk.DecCoins,
	listAsset, quoteAsset string, initPrice sdk.Dec, blockHeight uint64, maxPriceDigit, maxSizeDigit uint64, minTradeSize string) MsgDexListSubmitProposal {
...
func (msg MsgDexListSubmitProposal) ValidateBasic() sdk.Error {
	...
	if msg.ProposalType != ProposalTypeDexList {
		return ErrInvalidProposalType(DefaultCodespace, msg.ProposalType)
	}
	if msg.Proposer.Empty() {
		return sdk.ErrInvalidAddress(msg.Proposer.String())
	...
	//	return sdk.NewError(DefaultParamspace, CodeInvalidMergeTypes, fmt.Sprintf("MergeTypes %s", err.Error()))
	//}
...
func (msg MsgDexListSubmitProposal) String() string {
	return fmt.Sprintf("MsgSubmitProposal{%s, %s, %s, %v, %s, %s, %v, %v, %v, %v, %v}",
		msg.Title, msg.Description, msg.ProposalType, msg.InitialDeposit, msg.ListAsset, msg.QuoteAsset, msg.InitPrice, msg.BlockHeight, msg.MaxPriceDigit, msg.MaxSizeDigit, msg.MinTradeSize)
}
...
type MsgDeposit struct {
	...
	Amount     sdk.DecCoins   `json:"amount"`      // Coins to add to the proposal's deposit
}
...
func checkRange(str, maxUint, minFloat string) error {
	...
	if contMin == 0 {
		...
		midFloat = strings.Join([]string{"0.", strings.Repeat("0", contMin-1), "1"}, "")
	}
	contMax := strings.Count(maxUint, "0")
	if strings.Contains(str, ".") {
		strs := strings.Split(str, ".")
		if len(strs) != 2 {
			return fmt.Errorf("must be %s %s ... %s", minFloat, midFloat, maxUint)
		}
		cont0 := strings.Count(strs[1], "0")
		if cont0 > contMin {
			return fmt.Errorf("must be %s %s ... %s", minFloat, midFloat, maxUint)
		}
		expected := strings.Join([]string{"0.", strings.Repeat("0", cont0), "1"}, "")
		if expected != str {
			return fmt.Errorf("must be %s %s ... %s", minFloat, midFloat, maxUint)
		}
	} else {
		cont0 := strings.Count(str, "0")
		if cont0 > contMax {
			return fmt.Errorf("must be %s %s ... %s", minFloat, midFloat, maxUint)
		}
		expected := strings.Join([]string{"1", strings.Repeat("0", cont0)}, "")
		if expected != str {
			return fmt.Errorf("must be %s %s ... %s", minFloat, midFloat, maxUint)
		}
	}
