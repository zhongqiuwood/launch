...
const (
	...
	ParamMaxBlockHeightPeriod = "maxBlockHeightPeriod"
	ParamDexListDeposit       = "dexListDeposit"
	ParamDexListVoting        = "dexListVoting"
	ParamDexList              = "dexList"
)
...
func queryParams(ctx sdk.Context, path []string, req abci.RequestQuery, keeper Keeper) ([]byte, sdk.Error) {
	...
	case ParamTallying:
		...
		return bz, nil
	case ParamMaxBlockHeightPeriod:
		bz, err := codec.MarshalJSONIndent(keeper.cdc, keeper.GetParams(ctx).MaxBlockHeightPeriod)
		if err != nil {
			return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
		}
		return bz, nil
	case ParamDexListDeposit:
		bz, err := codec.MarshalJSONIndent(keeper.cdc, keeper.GetDexListDepositParams(ctx))
		if err != nil {
			return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
		}
		return bz, nil
	case ParamDexListVoting:
		bz, err := codec.MarshalJSONIndent(keeper.cdc, keeper.GetDexListVotingParams(ctx))
		if err != nil {
			return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
		}
		return bz, nil
	case ParamDexList:
		bz, err := codec.MarshalJSONIndent(keeper.cdc, keeper.GetDexListParams(ctx))
		if err != nil {
			return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
		}
		return bz, nil
