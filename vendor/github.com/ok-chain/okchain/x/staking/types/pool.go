...
type FormattedPool struct {
	NotBondedTokens sdk.Dec `json:"not_bonded_tokens"` // tokens which are not bonded to a validator (unbonded or unbonding)
	BondedTokens    sdk.Dec `json:"bonded_tokens"`     // tokens which are currently bonded to a validator
}
...
func (p Pool) String() string {
  ...
  Bonded Tokens: %s
  Token Supply:  %s
  Bonded Ratio:  %v`, p.NotBondedTokens.ToDec().QuoInt(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))),
		p.BondedTokens.ToDec().QuoInt(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))),
		p.TokenSupply(), p.BondedRatio())
}
