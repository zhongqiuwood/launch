...
type FormattedValidator struct {
	OperatorAddress         sdk.ValAddress `json:"operator_address"`    // address of the validator's operator; bech encoded in JSON
	ConsPubKey              crypto.PubKey  `json:"consensus_pubkey"`    // the consensus public key of the validator; bech encoded in JSON
	Jailed                  bool           `json:"jailed"`              // has the validator been jailed from bonded status?
	Status                  sdk.BondStatus `json:"status"`              // validator status (bonded/unbonding/unbonded)
	...
	UnbondingCompletionTime time.Time      `json:"unbonding_time"`      // if unbonding, min time for the validator to complete unbonding
	Commission              Commission     `json:"commission"`          // commission parameters
	MinSelfDelegation       sdk.Dec        `json:"min_self_delegation"` // validator's self declared minimum self delegation
}
...
func (v Validator) String() string {
  ...
  Commission:                 %s`, v.OperatorAddress, bechConsPubKey,
		v.Jailed, sdk.BondStatusToString(v.Status),
		v.Tokens.ToDec().QuoInt(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))),
		v.DelegatorShares, v.Description, v.UnbondingHeight, v.UnbondingCompletionTime,
		v.MinSelfDelegation.ToDec().QuoInt(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))),
		v.Commission)
}
...
func (v Validator) AddTokensFromDel(pool Pool, amount sdk.Int) (Validator, Pool, sdk.Dec) {
	...
	if v.DelegatorShares.IsZero() {
		...
		issuedShares = amount.Quo(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))).ToDec()
	} else {
...
func (v Validator) PotentialTendermintPower() int64 {
	return (v.Tokens.Quo(sdk.NewIntFromBigInt(new(big.Int).Mul(big.NewInt(1), new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil))))).Int64()
}
