...
func EndBlocker(ctx sdk.Context, k keeper.Keeper) ([]abci.ValidatorUpdate, sdk.Tags) {
	...
	for _, dvvTriplet := range matureRedelegationActions {
		err := k.CompleteRedelegationAction(ctx, dvvTriplet.DelegatorAddress,
...
func handleMsgCreateValidator(ctx sdk.Context, msg types.MsgCreateValidator, k keeper.Keeper) sdk.Result {
	...
	// fmt.Printf("msg amount: %v\n", msg.Value.Amount)
	// fmt.Printf("delegating amount: %v\n", msg.Value.Amount)
