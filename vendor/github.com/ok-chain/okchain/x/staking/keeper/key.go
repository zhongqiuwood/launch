...
var (
	...
	RedelegationActionKey      = []byte{0x91} // key for a redelegation action
	RedelegationActionQueueKey = []byte{0x92} // prefix for the height in redelegations action queue
	DelegatorPoolKey           = []byte{0x93} // key for delegator address pool
)
...
func getValidatorPowerRank(validator types.Validator) []byte {
	potentialPower := validator.Tokens.Quo(sdk.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(sdk.Precision), nil)))
...
// gets the key for a redelegation action
// VALUE: staking/types.RedelegationActionKey
func GetREDActionKey(delAddr sdk.AccAddress, valSrcAddr sdk.ValAddress) []byte {
	key := make([]byte, 1+sdk.AddrLen*2)

	copy(key[0:sdk.AddrLen+1], GetREDsActionKey(delAddr.Bytes()))
	copy(key[sdk.AddrLen+1:2*sdk.AddrLen+1], valSrcAddr.Bytes())
	return key
}

// gets the key for a delegator pool
// VALUE: staking/types.DelegatorPoolKey
func GetDelegatorPoolKey(delAddr sdk.AccAddress, valSrcAddr sdk.ValAddress) []byte {
	key := make([]byte, 1+sdk.AddrLen*2)

	copy(key[0:sdk.AddrLen+1], GetDelegatorsPoolKey(delAddr.Bytes()))
	copy(key[sdk.AddrLen+1:2*sdk.AddrLen+1], valSrcAddr.Bytes())
	return key
}
...
// gets the prefix for all unbonding delegations from a delegator based on block height
func GetRedelegationHeightKey(height int64) []byte {
	bz := sdk.Uint64ToBigEndian(uint64(height))
	return append(RedelegationActionQueueKey, bz...)
}
...
// gets the prefix keyspace for redelegation actions from a delegator
func GetREDsActionKey(delAddr sdk.AccAddress) []byte {
	return append(RedelegationActionKey, delAddr.Bytes()...)
}

// gets the prefix keyspace for delegator pool
func GetDelegatorsPoolKey(delAddr sdk.AccAddress) []byte {
	return append(DelegatorPoolKey, delAddr.Bytes()...)
}
