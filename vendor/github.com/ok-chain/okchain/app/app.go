...
import (
	...
	distr "github.com/ok-chain/okchain/x/distribution"
	"github.com/ok-chain/okchain/x/gov"
	"github.com/ok-chain/okchain/x/order"
	"github.com/ok-chain/okchain/x/staking"
...
type DexApp struct {
	*bam.BaseApp
	...
	keyValidatorsSnapShot *types.KVStoreKey
	keyDelegationSnapShot *types.KVStoreKey
...
func DecentralizedExchangeApp(logger log.Logger, db dbm.DB) *DexApp {
	...
	var app = &DexApp{
		...
		keyValidatorsSnapShot: sdk.NewKVStoreKey(distr.ValidatorsKey),
		keyDelegationSnapShot: sdk.NewKVStoreKey(distr.DelegationKey),
	...
	app.distrKeeper = distr.NewKeeper(
		app.cdc,
		app.keyDistr,
		app.keyValidatorsSnapShot,
		app.keyDelegationSnapShot,
...
func (app *DexApp) initChainer(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
	...
	for _, token := range genesisState.Token.Info {
		if token.Name != "" && token.Symbol != "" && token.TotalSupply > 0 {
			if token.Owner.Empty() && len(genesisState.Accounts) > 0 {
				token.Owner = genesisState.Accounts[0].Address
			}
...
func (app *DexApp) endBlocker(ctx sdk.Context, req abci.RequestEndBlock) abci.ResponseEndBlock {
	...
	tags.AppendTags(stakingTags)

	backend.EndBlocker(ctx, app.backendKeeper)
...
func (app *DexApp) beginBlocker(ctx sdk.Context, req abci.RequestBeginBlock) abci.ResponseBeginBlock {
	...
	distr.BeginBlocker(ctx, req, app.distrKeeper, app.feeCollectionKeeper)
...
func (app *DexApp) initFromGenesisState(ctx sdk.Context, genesisState GenesisState) []abci.ValidatorUpdate {
	...
	// 	panic(err) // TODO find a way to do this w/o panics
	// }
	app.distrKeeper.SetValidatorsSnapshoot(ctx)
