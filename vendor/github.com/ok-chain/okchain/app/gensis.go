...
func NewGenesisAccount(acc *auth.BaseAccount) GenesisAccount {
	return GenesisAccount{
		Address:       acc.Address,
		Coins:         sdk.NewDecCoins(acc.Coins),
...
func (ga *GenesisAccount) ToAccount() auth.Account {
	bacc := &auth.BaseAccount{
		Address:       ga.Address,
		Coins:         common.ConvertDecCoinsToCoins(ga.Coins.Sort()),
...
func CollectStdTxs(cdc *codec.Codec, moniker string, genTxsDir string, genDoc tmtypes.GenesisDoc) (
	...
	for _, fo := range fos {
		...
		if common.ConvertDecCoinsToCoins(delAcc.Coins).AmountOf(msg.Value.Denom).LT(msg.Value.Amount) {
			return appGenTxs, persistentPeers, fmt.Errorf(
		...
		for _, coin := range acc.Coins {
			if coin.Denom == genesisState.StakingData.Params.BondDenom {
				stakingData.Pool.NotBondedTokens = stakingData.Pool.NotBondedTokens.
					Add(sdk.NewIntFromBigInt(coin.Amount.Int)) // increase the supply
			}
		}
	}
